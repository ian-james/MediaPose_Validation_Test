...
def comparative_main(args, main_cap, second_cap, check_fps=False):
    ...
    out_record = open_recording_file(
        args['record'], (main_cap.width, main_cap.height), main_cap.fps_rate)
        ...
        while main_cap.cap.isOpened() and second_cap.cap.isOpened():
            ...
            if (check_fps == True):
                ...
                if (args['compare_file'] == args['filename']):
                    ...
                    frame1 = draw_mediapipe(
                        pose1, image1, total_frames, media_noface)
                    frame2 = draw_mediapipe(
                        pose2, image2, total_frames, media_noface)
                    frame2['video'] = 2
                ...
                if (args['compare_file'] == args['filename']):
                    ...
                    frame1 = setup_normal_frame(total_frames, fps, 1)
                    frame2 = draw_mediapipe_extended(
                        pose2, image2, total_frames, args['no_display'])
                    frame2['video'] = 2
                else:
                    # Different File with extended only, Do extended vs extended.
                    frame1 = draw_mediapipe_extended(
                        pose1, image1, total_frames, args['no_display'])
                    frame1['video'] = 1
                    frame2 = draw_mediapipe_extended(
                        pose2, image2, total_frames, args['no_display'])
                    frame2['video'] = 2
